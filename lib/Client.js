...
class Client extends Homey.SimpleClass {
	...
	constructor(app) {
		...
		this._compoundRegistrationEntityLookup = {};
	...
	addCompoundLookupTable(compoundId){
		let compound = this._compoundRegistration[compoundId];
	...
	removeCompoundLookupTable(compoundId){
		//let compound = this._compoundRegistration[compoundId];
		let keys = Object.keys(this._compoundRegistrationEntityLookup);
		for (let i=0; i<keys.length; i++){
			if (this._compoundRegistrationEntityLookup[keys[i]].compoundIds.find(id => id === compoundId)){
				this._compoundRegistrationEntityLookup[keys[i]].compoundIds = 
					this._compoundRegistrationEntityLookup[keys[i]].compoundIds.filter(id => id != compoundId)
			}
			if (this._compoundRegistrationEntityLookup[keys[i]].compoundIds.length <= 0){
				delete this._compoundRegistrationEntityLookup[keys[i]];
			}
		}
	}
			...
			if(data) {
				...
				if (entityLookup && entityLookup.compoundIds){
					for (let i=0; i<entityLookup.compoundIds.length; i++){
						let compound = this._compoundRegistration[entityLookup.compoundIds[i]];
						if (compound){
							await compound.device.onEntityUpdate(data.new_state);
						}
					}
	...
	async _onEntitiesUpdate(entities) {
		...
		if(currentLength != this._entitiesLength) {
			this._app.log("update entities: "+currentLength+" entries");

			this.countStatistics("entityUpdates");
			this.countStatistics("entityUpdatesElements", currentLength);
