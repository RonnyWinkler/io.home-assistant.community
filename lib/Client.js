...
class Client extends Homey.SimpleClass {
	...
	async unsubscribe(wait = false){
		if (this.isConnected()){
			if(this._connection != null) {
				this._connection.removeEventListener("ready", this._onEventConn);
				this._connection.removeEventListener("disconnected", this._onEventConn);
				this._connection.removeEventListener("reconnect-error", this._onEventConn);
	...
	async turnOnOff(entityId, on) {
		...
		try {
			...
			this._app.log("callService error:", error.error);
	...
	async sendMessage(message){
		...
		try {
			...
			this._app.log("sendMessage error:", error.message);
			throw new Error(error.error.message);
		}
	}
