...
class Client extends Homey.SimpleClass {
				...
				Object.keys(this._compoundRegistration).forEach(async (key) => {
					try {
						...
						if (compound.entityIds.indexOf(entityId) != -1){
							compound.device.onEntityUpdate(data.new_state);
						}
					} catch(e) {
						this._app.error("onStateChanged error:", e);
					}
				});
				// let keys = Object.keys(this._compoundRegistration);
				// for (let i=0; i<keys.length; i++){
				// 	let compound = this._compoundRegistration[keys[i]];
				// 	if (compound.entityIds.indexOf(entityId) != -1){
				// 		compound.device.onEntityUpdate(data.new_state);
				// 	}
				// }
	...
	// 							// Get base entity ID without attributes for comparing to changed HA entityId
	// 							let capabilityEntityId = capabilities[key];
	// 							let baseEntityId = capabilityEntityId.split(".")[0]+"."+capabilityEntityId.split(".")[1];
