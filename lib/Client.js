...
class Client extends Homey.SimpleClass {
	...
	constructor(app) {
		...
		this._unsubscribeEventsStateChange = null;
		this._unsubscribeEventsEvents = null;
		this._unsubscribeEventsAutomation = null;
	}
		...
		if (this.isConnected()){
			if(this._unsubscribeEntities != null){
				try{
					...
					this._unsubscribeEventsStateChange();
					this._unsubscribeEventsScript();
					this._unsubscribeEventsAutomation();
					if (wait){
						await this.sleep(2000);
					}
					this._unsubscribeEntities = null;
					this._unsubscribeEventsStateChange = null;
					this._unsubscribeEventsScript = null;
					this._unsubscribeEventsAutomation = null;
				}
				catch(error){
        ...
        // return new Promise((resolve, reject) => {
			...
			if(address && address != "" 
				...
				try{
					...
					this._unsubscribeEventsStateChange = await conn.subscribeEvents(this._onStateChanged.bind(this), "state_changed");
					this._unsubscribeEventsScript = await conn.subscribeEvents(this._onEventScript.bind(this), "script_started");
					this._unsubscribeEventsAutomation = await conn.subscribeEvents(this._onEventAutomation.bind(this), "automation_triggered");
	...
	_onEntitiesUpdate(entities) {
		...
		if(currentLength != this._entitiesLength) {
			...
			Object.keys(entities).forEach(id => {
				...
				if(entities[id] == undefined || entities[id] == null){
					return;
				}
				...
				try {
					...
					if(id.startsWith("climate.")) {
						let climateEntity = entities[id];
						let fanEntityId = "fan." + id.split(".")[1];
						let fanEntity = entities[fanEntityId];
			...
			this._fans = fans;
			this._climate_fans = climate_fans;
