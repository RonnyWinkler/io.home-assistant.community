...
class Client extends Homey.SimpleClass {
	...
	async _checkDeviceAvailability(){
		// Object.keys(this._deviceRegistration).forEach(async (key) => {
		// 	try {
		// 		let device = this._deviceRegistration[key];
		// 		if (this._entities[key] == undefined){
		// 			// Entity registered for device not available
		// 			// Set device unavailable
		// 			this._app.error("Set device unavailable: ", key);
		// 			await device.setUnavailable("Home Assistant entity not available");
		// 		}
		// 		else{
		// 			await device.setAvailable();
		// 		}
		// 	} catch(e) {
		// 		this._app.error("_checkDeviceAvailability error:", e);
		// 	}
		// });

		await this._app.checkDeviceAvailability();
	}
