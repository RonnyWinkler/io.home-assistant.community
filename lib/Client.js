...
class Client extends Homey.SimpleClass {
	...
	async _onEvent(event) {
		...
		try{
			...
			if (args["event_triggered_filter"]){
				let found = false;
				for (let i=0; i<args["event_triggered_filter"].length; i++){
					if ( 
						(args["event_triggered_filter"][i].event == '' || args["event_triggered_filter"][i].event == state.event) 
							&& 
						(args["event_triggered_filter"][i].entity == '' || args["event_triggered_filter"][i].entity == state.entity )
						){
						found = true;
					}
				}
	...
	async ping(){
		try {
			...
			this._app.log("Ping error:", error);
			throw error;
		}
	}
