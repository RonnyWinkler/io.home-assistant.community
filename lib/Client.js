...
class Client extends Homey.SimpleClass {
	...
	constructor(app) {
		...
		// Additional flow registration for entities (flow trigger for entity update)
		this._flowEntityRegistration = new Map();
	...
	registerFlowEntities(entityIds) {
		entityIds.forEach((entityId) => {
			this._flowEntityRegistration.set(entityId, true);
		});
	}

	unregisterFlowEntities(entityIds) {
		entityIds.forEach((entityId) => {
			this._flowEntityRegistration.delete(entityId);
		});
	}
	...
	getCapabilityTemplate(entityId, type=null){
		...
		if (entityAttribute){
			...
			switch (entityId.split('.')[0]){
				case "select":
				case "input_select":
					capabilityTemplate['capability'] = 'mode_select';
					break;					
				case "light":
				case "switch":
				case "input_boolean":
		...
		this._entities.forEach((entity, id) => {
			...
			try {
				if(id.startsWith("media_player.")) {
					...
					if ((features & 128) == 128 &&
						(features & 256) != 256) {
							mediaCapabilities.push("onoff_state");
							mediaCapabilities.push("on_button");
					}
					if ((features & 128) != 128 && 
						(features & 256) == 256) {
							mediaCapabilities.push("onoff_state");
							mediaCapabilities.push("off_button");
					}
					if ((features & 4) == 4) {
		...
		this._entities.forEach((entity, id) => {
			...
			try {
				if(id.startsWith("fan.")) {
					...
					if ((features & 8) == 8) {
						fanCapabilities.push("fan_mode_preset"); 
					}
					let fan = {
						name: entityName,
						data: {
			...
			try {
				if(id.startsWith("vacuum.")) {
					...
					if ((features & 32) == 32) {
						...
						capabilities.push("vacuum_mode_speed"); 
					}
					if ((features & 64) == 64) {
			...
			try {
				if(id.startsWith("climate.")) {
					...
					if (climateEntity != undefined && fanEntity != undefined){
						...
						if ((climateFeatures & 8) == 8) {
							climateCapabilities.push("climate_mode_fan");
						}
						if ((climateFeatures & 16) == 16) {
							climateCapabilities.push("climate_mode_preset");
						}
						if ((climateFeatures & 32) == 32) {
							climateCapabilities.push("climate_mode_swing");
						}
	...
	// CAMERA ============================================================================================ CLIMATE
	getLawnmowers(entityFilter = null) {
		this._app.log("Prepare LawnMower devices list.");
		let lawn_mowers = [];
		this._entities.forEach((entity, id) => {
			if(entity == undefined || entity == null){
				return;
			...
			try {
				if(id.startsWith("lawn_mower.")) {
					...
					// Source: 
					// https://github.com/home-assistant/core/blob/dev/homeassistant/components/lawn_mower/const.py
					// START_MOWING = 1
					// PAUSE = 2
					// DOCK = 4
				
					// Add standard capability. All features are just a value of the enum list of mower_state.
					// // default capabilities:
					// if ((features & 1) == 1) {
					// 	capabilities.push("onoff");
					// }
					capabilities.push("mower_state");
